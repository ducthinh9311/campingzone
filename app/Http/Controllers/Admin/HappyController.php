...
class HappyController extends Controller
{
     ...
     * @var string Paths
     */
    protected $viewPath = 'admin.happy';
    ...
    public function index()
    {
        ...
        foreach ($results as $item) {
            $listReview = DB::table('reviews')->where('product_id', $item->id)->get();
            $totalRating = 0;
            $totalReviews = 0;
            if (count($listReview) > 0) {
                ...
                $item->totalRating = round($totalRating/$totalReviews, 2);
                $item->totalReview = $totalReviews;
            }
        }
