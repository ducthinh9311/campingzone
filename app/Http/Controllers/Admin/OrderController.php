    ...
    public function update(Request $request, $id)
    {
        if (!hwa_check_permission('edit_order') || !$order = $this->order->with(['order_details'])->find($id)) {
            ...
            $validator = validator($request->all(), [
                ...
                'shipping_status.required' => 'Trạng thái giao hàng là trường bắt buộc.',
                'shipping_status.in' => 'Trạng thái giao hàng không hợp lệ.',
            ...
            if ($validator->fails()) {
                ...
                if($request['active'] == 'done'){
                    if( $request['payment_status'] == 0){
                        hwa_notify_error("Đơn hàng chưa thanh toán.");
                        return redirect()->back();
                    }
                    if($request['shipping_status'] != 2){
                        hwa_notify_error("Đơn hàng chưa giao thành công.");
                        return redirect()->back();
                    }
                }
                $order->fill([
